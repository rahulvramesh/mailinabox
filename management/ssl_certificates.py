	...
	for domain in plausible_web_domains:
		...
		# Check that the DNS resolves to here.
			...
			if bad_dns:
				...
				try:
					existing_cert = get_domain_ssl_files(domain, existing_certs, env)
...
def post_install_func(env):
	...
	wildcard_domain = re.sub("^[^\.]+", "*", env['PRIMARY_HOSTNAME'])
	if env['PRIMARY_HOSTNAME'] in ssl_certificates:
		cert = ssl_certificates[env['PRIMARY_HOSTNAME']]
	elif wildcard_domain in ssl_certificates:
		cert = ssl_certificates[wildcard_domain]
	else:
